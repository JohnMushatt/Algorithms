...
public class BST<Key extends Comparable<Key>, Value extends Comparable<Value>> {
	...
	public int numLeaves() {
		return leafFinder(this.root);
	}

	private int leafFinder(Node node) {
		// If we are called on an empty tree
		if (node == null) {
			return 0;
		}
		...
		if (node.left == null && node.right == null) {
			...
			return leafFinder(node.left) + leafFinder(node.right);
		}
	}
	 ...
	 */
	public Key maxValue() {
		Node result = maxNode(this.root, this.root);
		return result.key;
	}
