...
public class BandedArraySolution extends BandedArraySearch {
	...
	public int[] locate(int target) {
		...
		while (!bandFound) {
			int upper = inspect(0, mid);
			int lower = inspect(mid, 0);
			int middle = inspect(mid,mid);
			// If we are in the correct band
			if (target <= upper & target >= lower) {
				
				if(target>middle) {
					int start = 0;
					int midPoint, row, col, value;
					int end = n * n - 1;
					while (start <= end) {
						midPoint = start + (end - start) / 2;
						row = midPoint / n;
						col = midPoint % n;
						value = inspect(row, mid);
			...
			// If target is still in an outer band
			else if (target > upper) {
				mid = (mid + n) / 2;
			}
			// If target is still in a lower band
